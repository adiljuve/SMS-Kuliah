...
class Informatika extends Controller {
	...
	{
		$jurusan = '523';
		$data['class'] = 'informatika';
		$data['title'] = 'Teknik Informatika';
		$data['navsection'] = 'informatika';//link style
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'):
			...
			redirect('informatika/matakuliah');
			endif;
		endif;
	}
	...
	function mahasiswa()
	{
		...
		$this->load->view('template',$data);
 		...
 		if ($_SERVER['REQUEST_METHOD'] == 'POST'):
			...
			redirect('informatika/mahasiswa/'.$id);
		endif;
	}
	...
	function tambahMatakuliah()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'):
			...
			$this->Sms_model->tambahMatakuliah($data);
			redirect('informatika/tambahMatakuliah/'.$id);
			endif;
		endif;
	}
	...
	function editMatakuliahDosen()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'):
			...
			$this->Sms_model->edit_matakuliah($this->input->post('kode_matakuliah'),$data);		
			redirect('informatika/tambahMatakuliah/'.$this->input->post('dosen'));
		endif;
	}
	...
	function editMahasiswa()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'):
			...
			redirect('informatika/mahasiswa/'.$this->input->post('kode_matakuliah'));
		endif;	
	}
	...
	function confirmRequest()
	{
		...
		if($id==''){
			...
			$check = $this->Sms_model->cekDosen($password->password);
				if ($check!=0){
					echo "<script>window.alert('Penambahan Gagal! sudah ada dosen dengan password yang sama');</script><meta http-equiv=refresh content=0;url='".base_url()."informatika/request'>";
				}else{
